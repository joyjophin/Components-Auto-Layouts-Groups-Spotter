...
function analyzeFrames(frames) {
    ...
    let notComponents = 0;
    let groups = 0;
    function traverse(node, isInsideComponent = false) {
        totalLayers++;
        if (node.type === "GROUP") {
            groups++;
        }
...
function toggleHighlight(highlight, type) {
    if (highlight) {
...
function highlightNodes(nodes, type) {
    removeHighlights(type);
    const highlightRects = [];
    const processedNodes = new Set(); // To keep track of processed nodes
    function processNode(node, isInsideComponent = false) {
